      ...
      <div className="px-5 pb-5">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-accent transition-colors"
          >
            Preview
          </button>
          <button
            onClick={(e) => {
              e.stopPropagation();
              onUse(template.id, false, template.id);
            }}
            className="flex-1 rounded-xl bg-cyan-500 text-white px-4 py-2 text-sm font-medium hover:bg-cyan-600 transition-colors"
          >
            Use Theme
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-xl flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 bg-card/80 border-b border-border shadow-sm">
        ...
        <button
          onClick={onClose}
          className="p-2 text-muted-foreground hover:text-foreground bg-muted hover:bg-accent rounded-xl"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
    ...
    const killOverlays = () => {
      document.querySelectorAll('div, section, p, h2').forEach((el) => {
        ...
        if (
          ...
          const mainBox =
            el.closest('.fixed') ||
            el.closest('[class*="bg-background"]') ||
            el;
    ...
    setTimeout(() => {
      setA11yReport({
        ...
        issues: [
          ...
          {
            severity: 'serious',
            rule: 'Color contrast is insufficient',
            element: "<div class='text-gray-400 bg-white'>",
            suggestion: 'Increase contrast ratio to at least 4.5:1.',
          },
          {
            severity: 'moderate',
            rule: 'Heading levels skipped',
            element: '<h4>',
            suggestion: 'Use <h3> before <h4>.',
          },
          {
            severity: 'minor',
            rule: 'Redundant link text',
    ...
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
            ...
            <div className="mb-8 p-4 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/30 flex items-center justify-between">
              <div>
                <h3 className="text-emerald-400 font-bold flex items-center gap-2">
                  <Sparkles className="w-5 h-5" /> ✨ Resume Parsed
                  Successfully!
                </h3>
                <p className="text-sm text-muted-foreground mt-1">
                  Your data has been extracted. Select a template below and
                  we'll automatically inject your experience and projects!
                </p>
              </div>
              <button
                onClick={clearDraft}
          ...
          <div className="mt-12">
            <div className="mb-4 flex items-center gap-3 px-1">
              ...
              <h2 className="text-lg font-semibold text-foreground/70">
                Holographic Theme — About Section
              </h2>
            </div>
      ...
      <div className="mt-12">
        <div className="mb-4 flex items-center gap-3 px-1">
          ...
          <h2 className="text-lg font-semibold text-foreground/70">
            Playing Cards Theme — Click to flip, shuffle deck
          </h2>
        </div>
